...
import { ensureTweakccConfig, launchTweakccUi, runTweakcc, runTweakccAsync } from './tweakcc.js';
...
// Helper to yield to event loop between steps (allows React to re-render)
...
export const createVariantAsync = async (params: CreateVariantParams): Promise<CreateVariantResult> => {
  ...
  await report('Preparing directories...');
  const resolvedRoot = expandTilde(rootDir) ?? rootDir;
  const resolvedBin = expandTilde(binDir) ?? binDir;
  ...
  await report(`Installing ${resolvedNpmPackage}@${resolvedNpmVersion}...`);
  npmDir = path.join(variantDir, 'npm');
  ensureDir(npmDir);
  ...
  await report('Writing configuration...');
  const env = buildEnv({ providerKey, baseUrl, apiKey, extraEnv, modelOverrides });
  if (!Object.hasOwn(env, 'TWEAKCC_CONFIG_DIR')) {
    env.TWEAKCC_CONFIG_DIR = tweakDir;
  }
  ...
  await report('Setting up brand theme...');
  const brandKey = resolveBrandKey(providerKey, brand);
  ensureTweakccConfig(tweakDir, brandKey);
  ...
  if (providerKey === 'minimax') {
    await report('Configuring MiniMax MCP server...');
    ensureMinimaxMcpServer(configDir, resolvedApiKey);
  }
  const blockedZaiTools = providerKey === 'zai' ? ensureZaiMcpDeny(configDir) : false;
  if (blockedZaiTools) {
  ...
  let tweakResult: CreateVariantResult['tweakResult'] = null;
  if (!noTweak) {
    await report('Running tweakcc patches...');
    tweakResult = await runTweakccAsync(tweakDir, binaryPath, tweakccStdio);
    if (tweakResult.status !== 0) {
      const output = `${tweakResult.stderr ?? ''}\n${tweakResult.stdout ?? ''}`.trim();
      throw new Error(formatTweakccFailure(output));
    }
    ...
    if (promptPackEnabled) {
      await report(`Applying prompt pack (${promptPackModePreference})...`);
      const packResult = applyPromptPack(tweakDir, providerKey, promptPackModePreference);
      if (packResult.changed) {
        notes.push(`Prompt pack applied (${packResult.mode}, ${packResult.updated.join(', ')})`);
        await report('Re-applying tweakcc...');
        const reapply = await runTweakccAsync(tweakDir, binaryPath, tweakccStdio);
        tweakResult = reapply;
        if (reapply.status !== 0) {
          const output = `${reapply.stderr ?? ''}\n${reapply.stdout ?? ''}`.trim();
          throw new Error(formatTweakccFailure(output));
  ...
  await report('Writing CLI wrapper...');
  writeWrapper(wrapperPath, configDir, binaryPath, 'node');

  if (shellEnvEnabled && providerKey === 'zai') {
    await report('Configuring shell environment...');
    const shellResult = ensureZaiShellEnv({ apiKey: resolvedApiKey ?? null, configDir });
    if (shellResult.status === 'updated') {
      const suffix = shellResult.message ? ` (${shellResult.message})` : '';
      notes.push(`Z_AI_API_KEY written to ${shellResult.path}${suffix}`);
  ...
  if (skillInstallEnabled) {
    await report('Installing dev-browser skill...');
  ...
  await report('Finalizing variant...');
  const meta: VariantMeta = {
    name,
    provider: providerKey,
    baseUrl,
 ...
 */
export const updateVariantAsync = async (rootDir: string, name: string, opts: UpdateVariantOptions = {}): Promise<UpdateVariantResult> => {
  const report = async (step: string) => {
  ...
  if (!opts.noTweak) {
    ...
    tweakResult = await runTweakccAsync(meta.tweakDir, meta.binaryPath, commandStdio);
    if (tweakResult.status !== 0) {
      const output = `${tweakResult.stderr ?? ''}\n${tweakResult.stdout ?? ''}`.trim();
      throw new Error(formatTweakccFailure(output));
    }
    ...
    if (promptPackEnabled) {
      ...
      if (packResult.changed) {
        notes.push(`Prompt pack applied (${packResult.mode}, ${packResult.updated.join(', ')})`);
        await report('Re-applying tweakcc...');
        const reapply = await runTweakccAsync(meta.tweakDir, meta.binaryPath, commandStdio);
        tweakResult = reapply;
        if (reapply.status !== 0) {
          const output = `${reapply.stderr ?? ''}\n${reapply.stdout ?? ''}`.trim();
          throw new Error(formatTweakccFailure(output));
  ...
  await report('Updating configuration...');
